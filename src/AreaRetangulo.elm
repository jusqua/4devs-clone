...
areaRetangulo : String -> String  -> String
areaRetangulo b h =
  case (String.toFloat b, String.toFloat h) of
    (Just n, Just m) ->
      if n > 0 && m > 0 then String.fromFloat(n * m) ++ " u²"
      else "Indeterminado"
    _ -> "Indeterminado"
...
view model =
  let
    toText = {output = areaRetangulo model.i1 model.i2, title = "Calcular Área do Retângulo", box1 = "Base", box2 = "Altura"}
  in
