module AreaPoligonoRegular exposing (..)
import Browser
import Html exposing (..)
import Html.Events exposing (..)
import Html.Attributes exposing (..)

areaPoligonoRegular : String -> String -> String
areaPoligonoRegular lq l =
  case (String.toFloat lq, String.toFloat l) of
    (Just n, Just m) ->
      if n > 2 && m > 0 then String.fromFloat(((m^2)*n)/(4*tan(pi/n))) ++ " u²"
      else "Indeterminado"
    _ -> "Indeterminado"
