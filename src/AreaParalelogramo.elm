module AreaParalelogramo exposing (..)
import Browser
import Html exposing (..)
import Html.Events exposing (..)
import Html.Attributes exposing (..)

areaParalelogramo : String -> String -> String
areaParalelogramo b h = 
  case (String.toFloat b, String.toFloat h) of
    (Just n, Just m) ->
      if n > 0 then String.fromFloat(n*m) ++ " u²"
      else "Indeterminado"
    _ -> "Indeterminado"
...
view model =
  let
    toText = {output = areaParalelogramo model.i1 model.i2, title = "Calcular Área do Paralelogramo", box1 = "Base", box2 = "Altura"}
  in
